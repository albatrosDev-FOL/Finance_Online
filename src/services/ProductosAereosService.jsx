...
class ProductosAereosService {
  ...
  async getNationalAirports(token) {
    try {

      const response = await axios.get(`${this.url}Airport/NationalAirports`,
        {
      ...
      const errorMessage = error.response?.data?.message || error.message;
      console.error("Error al obtener el origen:", errorMessage);
      throw new Error("No se pudieron obtener el origen ");
    }

  }

  async getInternationalAirports(token) {
    try {

      const response = await axios.get(`${this.url}Airport/InternationalAirports`,
        { headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        },
      ...
      const errorMessage = error.response?.data?.message || error.message;
      console.error("Error al obtener el destino internacional:", errorMessage);
      throw new Error("No se pudieron obtener  destino internacional ");
    
    }}

  async getTaxesx(token,{IdSubProduct}){
   try{
    if( !IdSubProduct)
      throw new Error("Se requieren IDs:idProductType");
     const response = await axios.get(
        `${this.url}Tax/TaxesxSubProduct/${IdSubProduct}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
      ...
      let errorMessage = "Error al cargar impuesto";
      if (error.response) {
        errorMessage = error.response.data.Message || errorMessage;
      }
